              ...
              $_SESSION['store_id'] = $search_value;
               ...
               echo'<tr class="w3-sand"><th>Order ID</th><th>Quantity</th><th>Warehouse Location</th><th>Destination</th></tr>';
                 ...
                 while($row = mysqli_fetch_assoc($result)){
                   echo'<tr class=" w3-pink"><td>'.$row["Order_ID"].'</td><td>'.$row["Quantity"].'</td><td>'.$row["Warehouse_ID"].'</td><td>'.$row["Destination"].'</td></tr>';}
                 echo'</table>';
             }else{
               echo "0 results";
             }
...
$query = "UPDATE orders SET Warehouse_ID = '$warehouse_ID_chg' ,Destination = '$destination_chg' WHERE Order_ID='$store_id'";
